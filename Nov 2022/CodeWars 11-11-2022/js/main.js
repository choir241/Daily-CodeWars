...
// contiguous, same  number of divisors
// 14 ----> 1, 2, 7, 14 (4 divisors)
// 15 ----> 1, 3, 5, 15 (4 divisors)

//taking in array of subarrays and an integer

// diff is the difference between the 2 integers (diff = next - prev)
// nMax is max value of the range

//return the amount of pairs that exist that fulfills these properties


//for(let i = 1; i < nMax-1; i += diff)
...
function countPairsInt(diff, nMax) {
  let pairs = 0
    for(let i = 1; i < nMax-diff; i ++){
      if(numOfDivisors(i) == numOfDivisors(i+diff)){
        pairs++
      }
    }
  return pairs
